...
from tests.helpers import check_download_file
from browser_scripts.site_pages import SiteHelper
...
def test_scenario_one(browser: webdriver.Chrome) -> None:
    ...
    page.click_on_button(TENSOR_LOGO_BUTTON) # Пересмотреть локатор
    page.switch_tab(1)

    assert page.find_element(TENSOR_BUNNER).text == 'Сила в людях', 'Текст баннера не совпадает'

    page.click_on_button(ABOUT_BUTTON)
...
def test_scenario_two(browser: webdriver.Chrome) -> None:
    ...
    assert page.wait_element_by_text(REGION, "Камчатский край"), 'Регион был выбран неправильно'

    assert page.wait_element_by_text(PARTNER_BLOCK_REGION, "Петропавловск-Камчатский"),\
...
def test_scenario_three(browser: webdriver.Chrome) -> None:
    ...
    page.click_on_button(DOWNLOAD_LOCAL_VERSIONS_LINK)
    ...
    file_path = os.path.join(DOWNLOAD_PATH, FILE_NAME_PLUGIN)
    exist_file = check_download_file(file_path, 15)
